...
|---|---|---|
| `Canteen_Host_Server/` | เครื่องแม่ข่าย: ฐานข้อมูลนิสิต, นาฬิกา RTC, เว็บสำหรับเจ้าหน้าที่ | 113.0.0 |
| `Canteen_Station_Client/` | เครื่องประจำร้านค้า: อ่านบัตร RFID แล้วถามสิทธิ์จากแม่ข่าย | 122.0.0 |
...
```
Canteen_Host_Server/
├── Canteen_Host_Server.ino   ตรรกะระบบ ฐานข้อมูล ESP-NOW เส้นทางเว็บ   (1,889 บรรทัด)
├── HostScreen.h              ทุกอย่างที่วาดลงจอ TFT ของเครื่องแม่ข่าย      (647 บรรทัด)
└── WebDashboard.h            หน้าเว็บทั้งหมด: CSS, JavaScript, HTML        (593 บรรทัด)
```

`HostScreen.h` และ `WebDashboard.h` ถูก `#include` ไว้ **ท้ายไฟล์หลักก่อน `setup()`**
เพราะโค้ดข้างในอ้างถึงตัวแปรส่วนกลางที่ประกาศไว้ข้างบน **อย่าย้าย `#include` ขึ้นไปบนสุด**
Arduino IDE จะแสดงทั้งสามไฟล์เป็นแท็บของสเก็ตช์เดียวกัน เปิดพร้อมกันได้เลย

---

## อินเตอร์เฟส

ข้อความบนจอและบนหน้าเว็บ **เป็นภาษาอังกฤษทั้งหมด** เพราะนิสิตส่วนใหญ่เป็นชาวต่างชาติ
และคนที่ใช้งานจริงคือเจ้าของร้านค้า ไม่ใช่ช่าง ส่วนคำอธิบายในโค้ดยังเป็นภาษาไทย

มีสองโหมด **สว่าง** และ **มืด** เครื่องแม่ข่ายเป็นผู้กำหนดให้ทั้งระบบ
...
| หน้า | มีอะไร |
|---|---|
| **หน้าแรก** | มีประโยคเดียวที่นิสิตต้องอ่าน **TAP YOUR CARD** พร้อมยอดวันนี้และเวลา |
...
```sh
python3 tools/proto-check/bracecheck.py \
  Canteen_Host_Server/Canteen_Host_Server.ino \
