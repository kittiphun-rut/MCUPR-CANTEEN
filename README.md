...
| หน้า | มีอะไร |
|---|---|
| **หน้าแรก** | บอกสถานะการให้บริการสามแบบ **TAP YOUR CARD** (คลื่นเขียว) เมื่อพร้อม · **SERVER OFFLINE** (คลื่นเหลือง) เมื่อแม่ข่ายไม่ตอบ · **OUT OF SERVICE** (คลื่นแดง) เมื่อเครื่องอ่านบัตรไม่ตอบ พร้อมยอดวันนี้และเวลา |
...
| การ์ด SD (หลังโมดูลจอ, **บัส HSPI แยกของตัวเอง**) | SCK 6 · MISO 7 · MOSI 15 · CS 16 | 6–9 |
...
## เมื่อเครื่องแม่ข่ายไม่ตอบ

ระบบนี้ออกแบบให้ **fail-closed** คือถ้าตัดสินสิทธิ์ไม่ได้ก็ไม่ให้บริการ
ไม่ใช่เดาแล้วปล่อยผ่าน เพราะอาหารที่จ่ายไปแล้วเรียกคืนไม่ได้
และสิทธิ์เป็นวันละครั้งข้ามสี่จุดบริการ ถ้าแม่ข่ายล่มก็ล่มพร้อมกันทั้งสี่จุด
การอนุมัติเองตอนออฟไลน์จึงเปิดช่องให้รับซ้ำได้ทันทีโดยกระทบกันทีหลังไม่ได้

**จุดบริการไม่มีฐานข้อมูลเลย** ไม่เคยตัดสินใจเอง ส่งหมายเลขบัตรไปแล้วรอคำตอบอย่างเดียว

| สถานการณ์ | สิ่งที่จุดบริการทำ | ตัดสิทธิ์ไปหรือยัง |
|---|---|---|
| แม่ข่ายเงียบเกิน 15 วินาที | หน้าแรกขึ้น **SERVER OFFLINE** และ**ไม่รับการแตะบัตร** | **ไม่** การันตีได้ เพราะไม่ได้ส่งอะไรออกไป |
| แตะบัตรตอนนั้นพอดี | ขึ้นหน้าเตือน **SERVER OFFLINE** ทันที ไม่ต้องรอหมดเวลา | **ไม่** เหตุผลเดียวกัน |
| ส่งไปแล้วไม่มีคำตอบใน 3 วินาที | ขึ้นหน้า **RESULT UNKNOWN** | **ไม่ทราบ** ดูคำอธิบายข้างล่าง |

### ทำไมต้องมีคำว่า "ไม่ทราบผล"

ถ้าแม่ข่ายอนุมัติและบันทึกไปแล้ว แต่แพ็กเก็ตตอบกลับหายระหว่างทางทั้งสามครั้ง
จุดบริการจะหมดเวลารอโดยที่ **แม่ข่ายบันทึกว่ารับไปแล้วจริง ๆ**
การขึ้นว่า "ไม่สำเร็จ" ในกรณีนี้จึงเป็นการบอกข้อมูลผิด

เรื่องนี้แก้ให้หายขาดไม่ได้ในทางทฤษฎี เป็นปัญหา **Two Generals**
มาตรฐานจึงไม่พยายามกำจัด แต่ทำให้ **กู้คืนได้** แทน
หน้าจอจึงบอกว่าไม่ทราบผล แล้วชี้ทางให้ **แตะบัตรซ้ำ**
ถ้าแม่ข่ายบันทึกไปแล้วจะตอบว่า `ALREADY SERVED` ถ้ายังไม่ได้บันทึกก็จะตัดสิทธิ์ให้ตามปกติ
ไม่ว่าทางไหนก็ได้คำตอบที่ถูกต้อง และไม่มีทางจ่ายซ้ำ

### การส่งซ้ำไม่ทำให้ตัดสิทธิ์ซ้ำ

ทุกครั้งที่แตะบัตร จุดบริการแนบ **หมายเลขลำดับ (seq)** ไปด้วย และส่งซ้ำสูงสุดสามครั้ง
แม่ข่ายจำคู่ `(หมายเลขจุดบริการ, seq)` ล่าสุดไว้ ถ้าเจอคู่เดิมซ้ำ
มันจะ **ส่งคำตอบเดิมที่แคชไว้กลับไป ไม่ประมวลผลใหม่**
คุณสมบัตินี้เรียกว่า idempotent request เป็นหัวใจของการกันข้อมูลไม่ตรงกัน
ถ้าไม่มี การส่งซ้ำสามครั้งจะกลายเป็นตัดสิทธิ์สามครั้ง
