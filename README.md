...
|---|---|---|
| `Canteen_Host_Server/` | เครื่องแม่ข่าย: ฐานข้อมูลนิสิต, RTC, เว็บพอร์ทัล, เครื่องพิมพ์สลิป | 112.0.0 |
| `Canteen_Station_Client/` | เครื่องลูกข่ายประจำร้านค้า: อ่านบัตร RFID แล้วถามสิทธิ์จากแม่ข่าย | 121.0.0 |

### โครงสร้างไฟล์ของสเก็ตช์แม่ข่าย

```
Canteen_Host_Server/
├── Canteen_Host_Server.ino   ตรรกะของระบบ: ESP-NOW, ไฟล์ข้อมูล, เส้นทาง API   (2,674 บรรทัด)
├── HostDisplay.h             ทุกอย่างที่วาดลงจอ TFT ของเครื่องแม่ข่าย          (608 บรรทัด)
├── HostSlips.h               เนื้อหาสลิปและปลายทาง API ของเครื่องพิมพ์         (270 บรรทัด)
├── WebPortal.h               หน้าเว็บล้วน ๆ: getLoginHTML/getHTML/getDisplayHTML (1,836 บรรทัด)
└── ThermalPrinter.h          ไดรเวอร์เครื่องพิมพ์ 58 มม. (ESC/POS) USB OTG หรือ UART (329 บรรทัด)
```

Arduino IDE จะแสดงไฟล์ `.h` ทุกไฟล์เป็นแท็บของสเก็ตช์เดียวกันโดยอัตโนมัติ
ขอเพียงเก็บทั้งห้าไฟล์ไว้ในโฟลเดอร์เดียวกัน การคอมไพล์และการอัปโหลดทำเหมือนเดิมทุกอย่าง

**ไฟล์หลักถูกคุมไว้ไม่ให้เกิน 3,000 บรรทัด** ถ้าจะเพิ่มโค้ดก้อนใหญ่ ให้แยกเป็นไฟล์ `.h`
ใหม่แล้ว `#include` ไว้ตรงจุดเดียวกับไฟล์อื่น ๆ (ท้ายไฟล์ก่อน `setup()`)
การแยกไฟล์แบบนี้ **ไม่ได้ทำให้เฟิร์มแวร์เล็กลง** โค้ดเท่าเดิมคอมไพล์ออกมาได้ไบต์เท่าเดิม
แต่ช่วยให้อ่านง่ายขึ้นและเลี่ยงปัญหาต้นแบบฟังก์ชันอัตโนมัติของ Arduino ได้ด้วย
...
ภาพหน้าจอทั้งหมด 45 ภาพอยู่ใน [`docs/interface/`](docs/interface/) และเปิดดูรวมกันได้ที่
...
ตั้งแต่รุ่น 112.0.0 เครื่องแม่ข่ายจะ **ขึ้นจอบอกสถานะเครือข่ายให้เองตอนบูต**
ดูที่จอของเครื่องแม่ข่ายก่อนเสมอ

| สิ่งที่เห็นบนจอ | แปลว่า | ทำอย่างไร |
|---|---|---|
| แถบเขียว **WI-FI ACCESS POINT READY** พร้อมชื่อและรหัสผ่าน | เครื่องปล่อยสัญญาณแล้ว | ปัญหาอยู่ที่อุปกรณ์ที่ใช้ค้นหา ลองปิด-เปิด Wi-Fi ของโทรศัพท์ หรือลืมเครือข่ายเดิมแล้วค้นใหม่ |
| แถบแดง **WI-FI FAILED TO START** + เสียงเตือนต่ำสองครั้ง | เรียก `WiFi.softAP()` ไม่สำเร็จสามครั้งติด | ถอดไฟแล้วเสียบใหม่ ถ้ายังเป็นอีกให้แฟลชใหม่ |
| จอไม่ขึ้นอะไรเลย หรือค้างที่แอนิเมชันบูต | เครื่องแพนิกหรือบูตวน | ดูหัวข้อถัดไป |

**สาเหตุที่เคยเจอมาแล้วจริง ๆ สองข้อ แก้ไปแล้วทั้งคู่ในรุ่น 112.0.0**

1. **`WIFI_PROTOCOL_LR` บน SoftAP** — โหมดระยะไกลของ Espressif ทำให้โทรศัพท์และ
   คอมพิวเตอร์ **มองไม่เห็นชื่อ Wi-Fi** ของเครื่องแม่ข่าย ทั้งที่เครื่องปล่อยสัญญาณอยู่
   ตอนนี้ปิดเป็นค่าเริ่มต้นแล้ว (`#define ENABLE_WIFI_LONG_RANGE 0`)
2. **`printerBegin()` ถูกเรียกก่อนเปิด Wi-Fi** — ถ้าสแต็ก USB host ค้างหรือแพนิก
   (ซึ่งเกิดง่ายมากเมื่อยังไม่ได้จ่ายไฟ 5V เข้าขา VBUS) จะไม่มีชื่อ Wi-Fi โผล่มาเลย
   ตอนนี้ย้ายไปเรียกหลังเว็บเซิร์ฟเวอร์ขึ้นแล้ว และเครื่องพิมพ์ปิดเป็นค่าเริ่มต้นอยู่แล้ว
...
วัดจากการคอมไพล์จริงของรุ่น `release/08`

| | ไบต์ | หมายเหตุ |
|---|---|---|
| โปรแกรม (flash) | 1,339,228 | 43% ของพาร์ทิชัน 3 MB |
| ตัวแปรส่วนกลาง (RAM) | 53,576 | 16% ของ 320 KB |

> ตั้งแต่รุ่น 111.0.0 เป็นต้นไป **เครื่องพิมพ์สลิปถูกปิดไว้เป็นค่าเริ่มต้น**
> (`#define ENABLE_THERMAL_PRINTER 0`) ทั้งสแต็ก USB host และเนื้อหาสลิป
> จึงไม่ถูกคอมไพล์เข้าไปเลย เมื่อต่อเครื่องพิมพ์แล้วเปลี่ยนเป็น `1` ได้ทันที
> ตัวเลขในตารางข้างบนวัดตอนที่ยังเปิดอยู่ (รุ่น 110.0.2)

ส่วนที่กินแฟลชมากที่สุดคือ **หน้าเว็บที่ฝังอยู่ในเฟิร์มแวร์ 106 KB**
(พอร์ทัล 90 KB · จอสาธารณะ 12 KB · หน้าเข้าสู่ระบบ 4 KB)
รองลงมาคือไลบรารี `WebServer` + `ArduinoJson` + `Adafruit GFX/ST7789`
และสแต็ก USB host ของเครื่องพิมพ์

**ถ้าจำเป็นต้องลดขนาดจริง ๆ** (เช่นถูกบังคับให้ใช้พาร์ทิชัน 1.25 MB)
เรียงตามความคุ้มค่าและความเสี่ยง

| ทำอะไร | ประหยัดราว | เสียอะไรไป |
|---|---|---|
| ~~`#define ENABLE_THERMAL_PRINTER 0`~~ | 20–30 KB | **ทำไปแล้วตั้งแต่ 111.0.0 เป็นค่าเริ่มต้น** |
| บีบ HTML/CSS/JS (ตัดคอมเมนต์ ช่องว่าง) | ~13 KB | ไม่เสียฟังก์ชัน แต่โค้ดอ่านยากขึ้นมาก |
| ตัดแอตทริบิวต์สองภาษา `data-th`/`data-en` | ~6 KB | ปุ่มสลับ TH/EN บนเว็บใช้ไม่ได้ |
| ตัดหน้าจอสาธารณะ `/display` | ~15 KB | ต่อมอนิเตอร์จอใหญ่ให้นิสิตดูไม่ได้ |
| เลิกใช้ `ArduinoJson` เขียน JSON เอง | 15–25 KB | เสี่ยงพัง ต้องแก้ 4 จุด (admins, shops, students API) |
| ตัด `ESPmDNS` | 5–10 KB | เข้าเว็บด้วย `canteen.local` ไม่ได้ ต้องพิมพ์ IP |

ตัวเลขสองแถวแรกวัดจากซอร์สจริง ที่เหลือเป็นการประมาณ เพราะเครื่องที่ใช้พัฒนา
เข้า `downloads.arduino.cc` ไม่ได้ จึงคอมไพล์วัดเองไม่ได้
...
> พอเปลี่ยนเป็น *Huge APP* จะได้ 3 MB และใช้ไปเพียงราว 43%
>
...
```c
#define ENABLE_THERMAL_PRINTER 0   // ค่าเริ่มต้น = ปิด · เปลี่ยนเป็น 1 เมื่อต่อเครื่องพิมพ์แล้ว
#define PRINTER_TRANSPORT_UART 0   // 0 = USB OTG host, 1 = UART TTL
```

> **ค่าเริ่มต้นคือปิดไว้** เพราะสแต็ก USB host กินแฟลชราว 20–30 KB และไดรเวอร์
> คลาสปริ้นเตอร์ยังไม่เคยทดสอบกับเครื่องจริง ตอนปิดอยู่ระบบทำงานครบทุกอย่าง
> เพียงแต่ปุ่มพิมพ์ใบสรุปบนแดชบอร์ดจะไม่ขึ้น และแท็บตั้งค่าจะมีแถบเตือนบอกวิธีเปิด
