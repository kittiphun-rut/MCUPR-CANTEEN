 ...
 * | Version | Date | Change |
 * |---|---|---|
 * | 113.12.0 | 2026-09-24 | ย้ายการ์ด SD ไปอยู่บัส SPI ของตัวเอง (HSPI) ไม่ใช้ร่วมกับจอ สี่ขาเรียงติดกันที่ตำแหน่ง 6 ถึง 9 เดินสายหนึ่งต่อหนึ่งไม่ต้องแยกสาย และคืนบัสของจอให้เป็นสายขาออกอย่างเดียวเหมือนเดิม |
...
//    4        GPIO4     DS3231  SDA
//    5        GPIO5     DS3231  SCL
//    6        GPIO6     การ์ด SD  SD_SCK    (หัวต่อด้านหลังโมดูลจอ)
//    7        GPIO7     การ์ด SD  SD_MISO
//    8        GPIO15    การ์ด SD  SD_MOSI
//    9        GPIO16    การ์ด SD  SD_CS
//   10        GPIO17    บัซเซอร์
...
// การ์ด SD ที่อยู่ด้านหลังโมดูลจอ — สี่ขาเรียงติดกันที่ตำแหน่ง 6 ถึง 9
// โมดูลจอมีหัวต่อของการ์ดแยกไว้อีกชุดหนึ่งที่ขอบขวาของบอร์ด คือ
// SD_SCK, SD_MISO, SD_MOSI, SD_CS ไม่ได้ต่อกับหัวแถว SPI ของจอ ต้องเดินสายเอง
// ลำดับขาข้างล่างตรงกับลำดับบนตัวโมดูลพอดี เสียบตรงลงมาได้โดยไม่ไขว้
//
// [113.12.0] แก้: ย้ายการ์ดมาอยู่บัส SPI ของตัวเอง (HSPI) ไม่ใช้ร่วมกับจออีกแล้ว
//
//   ทั้งสองแบบใช้งานได้จริงและการแชร์บัสก็เป็นวิธีมาตรฐาน แต่เหตุผลที่แยก
//   1. **เดินสายง่ายกว่า** การแชร์บัสต้องเอาสายสองเส้นยัดเข้าหัวต่อเดียวกัน
//      ที่ GPIO13 กับ GPIO14 ซึ่งเป็นจุดที่หลุดง่ายที่สุดในงานประกอบด้วยมือ
//      แบบแยกคือสี่สายไปสี่ขาที่เรียงติดกัน หนึ่งต่อหนึ่ง
//   2. **สายหลุดเส้นเดียวไม่ดับทั้งคู่** ถ้าแชร์บัสแล้ว SCK หลุด จอกับการ์ดดับพร้อมกัน
//   3. **บัสของจอสะอาดเหมือนเดิม** ไลบรารี ST77xx ตั้งความเร็วไว้ที่ 32 MHz
//      การแตกแขนงสายเพิ่มที่ความถี่นั้นทำให้สัญญาณแย่ลง
//
//   ที่ทำได้เพราะแม่ข่ายใช้ SPI แค่ตัวเดียวคือ FSPI ส่วน HSPI ว่างสนิท
//   และขาตำแหน่ง 8 กับ 9 ก็ว่างอยู่พอดี เหตุผลที่ตำราส่วนใหญ่ให้แชร์บัส
//   คือบอร์ดอย่าง Arduino Uno มีขาไม่พอ ซึ่งไม่ใช่ข้อจำกัดของบอร์ดนี้
#define SD_SCK_PIN          6
...
// [113.12.0] เพิ่ม: การ์ด SD อยู่บนบัสของตัวเอง ไม่ยุ่งกับบัสของจอเลย
...
bool initSDCard() {
  ...
  // อยู่คนละบัสกับจอ จึงไม่ต้องไปคิดเรื่องความเร็วของอีกฝ่ายเลย
  SPI_SD.begin(SD_SCK_PIN, SD_MISO_PIN, SD_MOSI_PIN, SD_CS_PIN);
  if (!SD.begin(SD_CS_PIN, SPI_SD, 10000000)) return false;
...
void setup() {
  ...
  // [113.12.0] แก้: คืนกลับเป็นไม่มี MISO เพราะการ์ด SD ย้ายไปบัสของตัวเองแล้ว
  //           จอไม่เคยต้องอ่านข้อมูลกลับ บัสนี้จึงมีแต่สายขาออกอย่างเดียว
